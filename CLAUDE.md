...
The library itself is a handful of source files; `Examples/` holds runnable showcase executables.

```sh
swift run report                 # one example to stdout (also: invoice, article, gallery)
Examples/render.sh [outdir]      # render all examples → outdir (default docs/, local preview only)
```

**CI / Pages**: `.github/workflows/pages.yml` runs on every push to `main` — it `swift test`s, renders the examples into `_site/`, screenshots them (headless Chrome → `.github/scripts/trim.py` trims to content with Pillow), and deploys the lot to **GitHub Pages** (`alleato-llc.github.io/kumi`). So the gallery, pages, **and the README's screenshots are all generated by the pipeline, never committed** — `docs/` is gitignored (local preview only), and the README links to the Pages-hosted `…/screenshots/*.png`. Don't commit `docs/` or screenshots; change `Examples/` and the pipeline refreshes everything (and proves the examples still compile).

`Examples/{report,invoice,article,gallery}` are **`.executableTarget`s**, not library products — so packages depending on `Kumi` never build them. Each prints a full styled page to stdout; `render.sh` writes them into `docs/` (the gallery's `index.html` is itself built with Kumi). They compile against the live API, so an API change that breaks an example fails the build (in CI) — which is the point. The rendered HTML/screenshots are produced by the pipeline, not committed.
