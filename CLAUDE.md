...
- **`Node.swift`** — the public `Node` value type (an enum-backed `Storage`: `.element`/`.text`/`.raw`/`.fragment`), the static factories (`tag`/`text`/`raw`/`fragment`/`document`), and `render()`. `render()` is the *only* place markup is emitted. Also the `voidElements` set (`<br>`, `<meta>`, … — no closing tag) and `[Node].render()`.
